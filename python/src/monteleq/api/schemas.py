...
# ---------------------------------------------------------------------------
# Pending ingestion requests — one row per curve×window×variant
# ---------------------------------------------------------------------------

PENDING_REQUESTS_SCHEMA = schema(
    fields=[],
    metadata={
        "comment": (
            "Pending ingestion requests generated by the plan stage. "
            "Each row represents a single fetch unit: one curve, one time window, "
            "optionally with ensembles or unit override."
        ),
    },
    tags={
        "domain": "energy",
...
PENDING_REQUESTS_SCHEMA["request_id"] = schema_field(
    "request_id", pa.string(), nullable=False,
    metadata={"comment": "Unique request identifier (curve_name|variant|begin|end)"},
    tags={"primary_key": "true"},
)

PENDING_REQUESTS_SCHEMA["curve_name"] = schema_field(
    "curve_name", pa.string(), nullable=False,
    metadata={"comment": "EnergyQuantified curve name"},
)

PENDING_REQUESTS_SCHEMA["curve_type"] = schema_field(
    "curve_type", pa.string(), nullable=False,
    metadata={"comment": "EnergyQuantified curve type"},
)

PENDING_REQUESTS_SCHEMA["data_type"] = schema_field(
    "data_type", pa.string(), nullable=False,
    metadata={"comment": "Vendor data type"},
)

PENDING_REQUESTS_SCHEMA["table_category"] = schema_field(
    "table_category", pa.string(), nullable=False,
    metadata={"comment": "Target Delta table category"},
    tags={"group": "routing"},
)

PENDING_REQUESTS_SCHEMA["cluster_key"] = schema_field(
    "cluster_key", pa.string(), nullable=False,
    metadata={"comment": "Cluster group key (data_type_curve_type)"},
    tags={"group": "routing"},
)

PENDING_REQUESTS_SCHEMA["begin"] = schema_field(
    "begin", pa.timestamp("us", "UTC"), nullable=False,
    metadata={"comment": "UTC start of the fetch window"},
)

PENDING_REQUESTS_SCHEMA["end"] = schema_field(
    "end", pa.timestamp("us", "UTC"), nullable=False,
    metadata={"comment": "UTC end of the fetch window"},
)

PENDING_REQUESTS_SCHEMA["ensembles"] = schema_field(
    "ensembles", pa.bool_(), nullable=False,
    metadata={"comment": "Whether this request fetches ensemble data"},
)

PENDING_REQUESTS_SCHEMA["unit"] = schema_field(
    "unit", pa.string(), nullable=True,
    metadata={"comment": "Unit override (e.g. EUR conversion)"},
)

PENDING_REQUESTS_SCHEMA["mode"] = schema_field(
    "mode", pa.string(), nullable=False,
    metadata={"comment": "Insert mode (append, overwrite, upsert)"},
)

PENDING_REQUESTS_SCHEMA["created_at"] = schema_field(
    "created_at", pa.timestamp("us", "UTC"), nullable=False,
    metadata={"comment": "UTC timestamp when the request was generated"},
)
